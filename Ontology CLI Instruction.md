...
--config   
The config parameter specifies the file path of the genesis block for the current Ontolgy node. The default value is "./config.json". Note that the genesis block configuration must be the same for all nodes in the same network, otherwise it will not be able to synchronize blocks or start nodes due to block data incompatibility.
...
--testmode  
The testmode parameter is used to start a single node test network for ease of development and debug.
...
For deploying node, we need to provide the genesis block configuration file for initializing the genesis block. Ontology supports the VBFT and dBFT consensus algorithms. A blockchain network must use the same consensus algorithm, otherwise nodes will not work properly. If you access the Ontology public chain, you can use the default configuration file and do not need to modify the genesis block configuration file. However, if you need to build an Ontology private chain, you need to modify the corresponding configuration items of the configuration file, such as the seed list and public key of node.  
...
#### 2.5.1Import Account Parameters
