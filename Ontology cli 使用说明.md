...
Ontology cli 是ontology节点命令行客户端，用于启动和管理ontology节点，管理用户钱包，发送交易以及部署和调用合约等。

## 1、启动和管理Ontology节点

Ontology cli有很多启动参数，用于配置Ontology节点的一些行为。使用./ontology -help 可以查看到Ontology cli节点支持的所有启动参数。如果不带任何参数启动ontolgy cli时，默认会以同步节点的身份接入 Ontology的polaris测试网络。
...
--config   
config 参数用于指定当前Ontolgy节点的创世区块配置文件路径。默认值为"./config.json"。注意，同一个网络所有节点的创世区块配置必须一致，否则会因为区块数据不兼容导致无法同步区块或启动节点。

--loglevel   
loglevel 参数用于设置Ontology输出的日志级别。Ontology支持从0:Debug 1:Info 2:Warn 3:Error 4:Fatal 5:Trace 6:MaxLevel 的7级日志，日志登记由低到高，输出的日志量由多到少。默认值是1，即只输出info级以及更高级别的日志。
...
--importheight    
importheight 参数配合--import使用，用于指定导入的目标区块高度。如果importheight指定的区块高度小于区块文件的最大高度时，只导入到importheight指定的高度，剩余的区块会停止导入。默认值为0，表示导入所有的区块。
...
--testmode  
testmode 参数用于启动单节点的测试网络，便于开发和调成。

--testmodegenblocktime  
testmodegenblocktime 参数用于设置测试模式下的出块时间，时间单位为秒，最小出块时间为2秒。

#### 1.1.9 交易参数

--gasprice  
gasprice 参数用于设定当前节点交易池接受交易的最低gasprice，低于这个gasprice的交易将会被丢弃。默认值为0。

--gaslimit   
gaslimit 参数用于设置当前节点交易池接受交易的最近gaslimit，低于这个gaslimit的交易将被丢弃。默认值为30000。
...
部署节点需要提供创世区块的配置文件，用于初始化创世区块。Ontology支持VBFT和dBFT共识算法，一个区块链网络必须使用同一种共识算法，否则节点将无法正常运行。如果接入的是Ontology公链，可以使用默认的配置文件，不需要修改创世区块配置文件；但是如果需要搭建Ontology私有链，则需要修改配置文件相应的配置项，如种子列表，节点公钥等信息。
...
按照节点角色不同，可以分为记账节点和同步节点，记账节点参与网络共识，而同步节点只同步记账节点生成的区块。Ontology节点默认会启动Rpc服务器，同时会输出智能合约输出的Event Log，因此如果没有特殊要求，可以使用--disablerpc和--disableeventlog命令行参数关闭rpc和eventlog模块。
...
Ontology支持单节点网络部署，用于开发测试环境搭建。启动单节点测试网络只需要加上--testmode命令行参数即可。
...
可以使用 ./ontology account --help 命令可以查看钱包管理命令的帮助信息。

### 2.1、添加账户

Ontology支持多种加密算法，包括ECDSA、SM2以及ED25519等。
...
在使用SM2加密算法时，将会使用sm2p256v1曲线，同时使用SM3withSM2签名算法。
...
--bit-length,b  
bit-length参数用于指定密钥长度，如果时ecdsa加密算法，可以选择p-224, p-256, p-384, p-521；如果是sm2加密算法，默认为sm2p256v1；如果是ed25519加密算法，默认为25519。
...
--label  
label用于给新创建的账户设置标签，用于方便、快捷使用账户。注意，同一个钱包文件下，不能出现重复的label名。没设置label的账户则为空字符串。
...
可以通过 ./ontology account add --help 来查看帮助信息。
...
使用--v 可以查看账户的详细信息。可以通过 ./ontology account list --help 来查看帮助信息。

### 2.3 修改账户

使用修改账户命令可以修改账户的标签，重新设置默认账户，修改账户密码，如果账户是ECDSA加密算法的密钥，还可以修改密钥的签名方案。可以通过 ./ontology account add --help 来查看帮助信息。

#### 2.3.1修改账户参数

--as-default, -d  
as-default参数设置账户为默认账户。一个钱包只有有一个默认账户，设置了新的默认账户后，之前的默认账户会自动取消默认账户属性。

--wallet, -w   
wallet参数指定当前操作的钱包路径，默认值为"./wallet.dat"。

--label, -l  
label参数用于给账户设置新的标签。注意一个钱包文件中，不能有相同的lable。
...
资产管理命令可以查看账户的余额，ONT/ONG转账，提取ONG以及查看未绑定的ONG等。
...
用户可以授权其他用去从本账户中转账，授权转账时可以指定转账额度。
...
--sender
sender参数指定实际执行授权转账的账户地址。如果没有指定sender参数，sender参数默认使用to参数的值。
...
ONG采用定时解绑策略解除绑定在ONT上的ONG。使用如下命令可以查看到当前账户为解绑的ONG余额。
...
查询信息命令可以查询区块、交易以及交易执行等信息。可以使用./ontology info block --help 命令查看帮助信息。
...
智能部署前需要在NeoVM合约编译器如：[SmartX](http://smartx.ont.io) 上编译好之后，把编译后的Code保存在本地的一个文本文件中。
...
--gasprice  
gasprice参数指定部署合约交易的gas price。交易的gas price不能小于接收节点交易池设置的最低gas price，否则交易会被拒绝。默认值为0。当交易池中有交易在排队等待打包进区块时，交易池会按照gas price有高到低排序，gas price高的交易会被优先处理。

--gaslimit  
gaslimit参数指定部署合约交易的gas limit。交易的gas limit不能小于接收节点交易池设置的最低gas limit，否则交易会被拒绝。gasprice * gaslimit 为账户实际支持的ONG 费用。

**对于合约部署，gaslimit 值必须大于10000000，同时账户中必须保有足够的ONG余额。**

--needstore 
needstore参数指定智能合约属否需要使用持久化存储，如果需要需要带上该参数。默认为不使用。
...
如果返回的执行状态State等于0，表示交易执行失败，如果State等于1，表示交易执行成功，合约部署成功。如：
...
NeoVM智能合约支持array、bytearray、string、int以及bool参数类型。其中array表示对象数组，可以嵌套NeoVM支持的任意数量、任意类型的参数；bytearray表示字节数组，输入时需要先十六进制编码成字符串，如 []byte("HelloWorld") 编码成：48656c6c6f576f726c64；string表示字符串字面值；int表示整数，由于NeoVM虚拟机不支持浮点数值，因此需要将浮点数转换成整数；bool表示布尔型变量，用true，false表示。

在Ontology cli中，使用前缀法构造输入参数，参数前使用类型标识标注类型，如字符串输入参数表示为 string:hello; 整数参数表示为 int:10; 布尔类型参数表示为 bool:true等。多个参数使用","分隔。对象数值array类型用"[ ]", 表示数组元素范围，如 [int:10,string:hello,bool:true]。
...
--prepare, -p  
prepare参数表示当前为预执行合约，执行交易不会被打包到区块中，也不会消耗任何ONG。预执行会返回合约方法的返回值，同时还会试算当前调用需要的gas limit。

--return 
return参数用于配合--prepare参数使用，在预执行时通过--return参数标注的返回值类型来解析合约返回返回值，否则输出合约方法调用的返回的原始值。多个返回值类型用","分隔，如 string,int
...
Ontology Cli支持导出本地节点的区块数据到一个压缩文件中，生成的压缩文件可以在导入Ontology节点中。出于安全考虑，导入的区块数据文件请确保是从可信的来源获取的。
